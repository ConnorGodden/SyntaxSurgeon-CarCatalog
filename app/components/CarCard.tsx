...
import { formatMissingFieldSummary, getMissingListingFields, isListingIncomplete } from "../../utils/listingCompleteness";
...
export default function CarCard({ car }: { car: Car }) {
    ...
    return (
      <div className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-800 w-full h-100 transition-transform duration-150 ease-out hover:-translate-y-0.5 hover:shadow-lg hover:shadow-zinc-900/10 dark:hover:shadow-black/30">
        ...
        {incomplete && (
          <div className="mb-2 rounded-md border border-amber-300 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-800 dark:border-amber-700/60 dark:bg-amber-950/30 dark:text-amber-200">
            Incomplete listing: {formatMissingFieldSummary(missingFields)}
          </div>
        )}
        <h2 className="text-base font-semibold">{car.make}</h2>
        <p className="text-sm text-zinc-500">{car.model}</p>
        <p className="mt-1 text-zinc-500">{Number.isFinite(car.year) ? car.year : "Missing year"}</p>
